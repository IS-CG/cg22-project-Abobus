...
@njit
def padding(image: np.ndarray,
            padding_size: int) -> np.ndarray:  # todo перенеси вместо своего паддинга, я не хочу все твои алгосы рефакторить :3
    ...
    def _bilinear_resize(image: np.ndarray, new_width: int, new_height: int) -> np.ndarray:
        ...
        for i in range(new_height):
            for j in range(new_width):
                ...
                x_diff = x - x1
                y_diff = y - y1
                new_image[i, j] = (1 - x_diff) * (1 - y_diff) * image[y1, x1] + x_diff * (1 - y_diff) * image[
                    y1, x2] + (
